...
app.use(
  cors({
    origin: [
      ...
      "https://assignment-11-4d65a.firebaseapp.com",
    ],
    credentials: true,
  })
);
  ...
  res.send(result);
    ...
    app.post("/registrations", async (req, res) => {
      ...
      if (existingRegistration) {
        return res
          .status(400)
          .send({ message: "You have already registered for this marathon." });
      }
    ...
    app.put("/registrations/:id", async (req, res) => {
      const { id } = req.params;
      const { marathonTitle, startDate, firstName, lastName, contactNumber } =
        req.body;
      ...
      try {
        ...
        if (result.modifiedCount === 0) {
          return res
            .status(404)
            .json({ message: "Registration not found or no changes made." });
        }
